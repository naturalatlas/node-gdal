...
Handle<Value> Feature::getGeometry(const Arguments& args)
{
  ...
  if(!geom) NODE_THROW("Error getting feature geometry");
...
Handle<Value> Feature::getFieldAsIntegerList(const Arguments& args)
{
  ...
  if(count_of_values < 0) return NODE_THROW("Invalid list length");
...
Handle<Value> Feature::getFieldAsBinary(const Arguments& args)
{
  ...
  if(count_of_bytes < 0) return NODE_THROW("Invalid buffer length");
...
Handle<Value> Feature::getField(const Arguments& args)
{
  ...
  switch(field_def->GetType()){
     	case OFTInteger:        return feature->getFieldAsInteger(args);
    	case OFTIntegerList:    return feature->getFieldAsIntegerList(args);
    	case OFTReal:           return feature->getFieldAsDouble(args);
    	case OFTRealList:       return feature->getFieldAsDoubleList(args);
    	case OFTString:         return feature->getFieldAsString(args);
    	case OFTStringList:     return feature->getFieldAsStringList(args);
     	case OFTBinary:         return feature->getFieldAsBinary(args);
    	case OFTDate:           return feature->getFieldAsDateTime(args);
    	case OFTTime:           return feature->getFieldAsDateTime(args);
    	case OFTDateTime:       return feature->getFieldAsDateTime(args);
      default:  return NODE_THROW("Unsupported field type in getFieldValue method");
  }
}
